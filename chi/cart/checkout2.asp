...
<%
	...
	If Session("ShipCountry") <> "" And Session("ShipProvince") <> "" And Session("SubTotalCost") > 0 Then
		...
		For t = 1 to Session("NumCartItems")
            ...
            If Left(Session("CartItem(" & t & ")"), 4) = "C030" Or InStr(Session("CartItem(" & t & ")"), "W1141") > 0 Then   
                'ChangePricing = True
            End If
...
'    End If
	...
	For t = 1 to Session("NumCartItems")
	    If DateDiff("d", Date, CDate("2020/12/31")) < 0 Then
		    If Len(Session("CartItem(" & t & ")")) = 5 And Session("CartItem(" & t & ")") <> "C302X" And Left(Session("CartItem(" & t & ")"), 1) = "C" And Right(Session("CartItem(" & t & ")"), 1) = "X" And (Not IsNull(Session("CurrentTransaction")) And Left(Session("CurrentTransaction"), 1) = "Q")  Then
	    ...
	    End If
            ...
            If Session("CartItem(" & t & ")") = "C030" And IsEmpty(Session("ConferenceInterests")) Then
